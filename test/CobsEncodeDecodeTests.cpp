...
// The fixture for testing class Foo.
    ...
    TEST_F(CobsEncodeDecodeTest, IncorrectDecodeTest) {
        ...
        std::string decodedData;
        SerialFiller::DecodeStatus decodeStatus = SerialFiller::CobsDecoder(std::string({0x02, 0x00, 0x00}), decodedData);
    ...
    TEST_F(CobsEncodeDecodeTest, IncorrectDecodeTest2) {
        ...
        std::string decodedData;
        SerialFiller::DecodeStatus decodeStatus = SerialFiller::CobsDecoder(std::string({0x02, '\xAA', 0x03, 0x00, '\xAB'}), decodedData);
        EXPECT_EQ(SerialFiller::DecodeStatus::ERROR_ZERO_BYTE_NOT_EXPECTED, decodeStatus);
        EXPECT_EQ(std::string(), decodedData);
    }
