...
namespace {
    ...
    TEST_F(SplitPacketTests, BasicTest) {
        ...
        std::string packetAsString = { 0x04, 't', 'e', 's', 't', 'h', 'e', 'l', 'l', 'o', 0x01, 0x01};
    ...
    TEST_F(SplitPacketTests, BogusTopicLength) {
        auto packet = ByteArray({ 0x06, 0x02, 0x03 });
        auto topic = std::string();
        auto data = ByteArray();
        EXPECT_THROW(SerialFillerHelper::SplitPacket(packet, topic, data), LengthOfTopicTooLong);
        EXPECT_EQ("", topic);
